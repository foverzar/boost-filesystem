 ...
 * file system.
 *
 * Class `path` supports a variety of methods for observing and modifying the path. Among the modifiers,
 * there are two groups of methods for performing concatenation and appending. The important distinction
 * between these two groups is that concatenation is performed on the path as if the path was a simple
 * string (i.e. the concatenated characters are placed immediately at the end of the path), while appending
 * maintains path structure and may insert a directory separator before the appended characters. The rules
 * for inserting the separator depend on the library version and are documented in the \ref path::append
 * method description.
 *
 * Class `path` supports iterators over its elements:
 *
 * <pre>
 * [root-name?] [root-directory?] [filename*]
 * </pre>
 *
 * Note that dereferencing a path iterator produces a path.
 *
 * \par `path` imbued locale
 *
 * `path` operations sometimes require encoding conversions between pathname and some other string object
 * where one of the value types is `path::value_type` and the other is another path character type. Such
 * conversions shall be performed by the `path::codecvt()` facet by default, unless another facet is
